...
export function ProductsPageContent() {
  ...
  return (
    <div>
      <CategoryFilter categories={Array.from(new Set(products.map((p) => p.category)))} selectedCategory={selectedCategory} onSelectCategory={setSelectedCategory} />
      {filteredProducts.length > 0 ? <ProductGrid products={filteredProducts} /> : <p className="mt-8 text-center text-gray-500">No products found.</p>}
    </div>
  );
}
